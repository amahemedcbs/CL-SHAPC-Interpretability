...
from models.PyCIL.convs.ACL_buffer import RandomBuffer, activation_t
from models.PyCIL.convs.linears import (
    CosineLinear,
    RecursiveLinear,
    SimpleLinear,
    SplitCosineLinear,
...
from models.PyCIL.convs.memo_cifar_resnet import (
    get_resnet32_a2fc as get_memo_resnet32,  # for MEMO cifar
)
from models.PyCIL.convs.memo_cifar_resnet import (
    get_resnet32mnist_a2fc as get_memo_resnet32mnist,  # for MEMO mnist
)
from models.PyCIL.convs.memo_resnet import (
    get_resnet18_imagenet as get_memo_resnet18,  # for MEMO imagenet
)
from models.PyCIL.convs.modified_represnet import resnet18_rep, resnet34_rep
from models.PyCIL.convs.resnet_cbam import (
    resnet18_cbam,
    resnet34_cbam,
    resnet50_cbam,
)
from models.PyCIL.convs.ucir_resnet import resnet18 as cosine_resnet18
from models.PyCIL.convs.ucir_resnet import resnet34 as cosine_resnet34
from models.PyCIL.convs.ucir_resnet import resnet50 as cosine_resnet50
...
class DERNet(nn.Module):
  ...
  def forward(self, x):
    ...
    out_raw = self.fc(features)
    logits = out_raw["logits"] if isinstance(out_raw, dict) else out_raw

    aux_raw = self.aux_fc(features[:, -self.out_dim :])
    aux_logits = aux_raw["logits"] if isinstance(aux_raw, dict) else aux_raw

    out = {"logits": logits, "aux_logits": aux_logits, "features": features}
...
class MultiBranchCosineIncrementalNet(BaseNet):
  ...
  def __init__(self, args, pretrained):
    ...
    print(
        "Clear the convnet in MultiBranchCosineIncrementalNet, since we are"
        " using self.convnets with dual branches"
    )
  ...
  def update_fc(self, nb_classes, nextperiod_initialization=None):
    ...
    if self.fc is not None:
      ...
      if nextperiod_initialization is not None:
        ...
        weight = torch.cat(
            [weight, torch.zeros(nb_classes - nb_output, self._feature_dim).cuda()]
        )
      fc.weight = nn.Parameter(weight)
    del self.fc
    self.fc = fc
...
class FOSTERNet(nn.Module):
  ...
  def forward(self, x):
    ...
    fe_raw = self.fe_fc(features[:, -self.out_dim :])
    fe_logits = fe_raw["logits"] if isinstance(fe_raw, dict) else fe_raw
    ...
    if self.oldfc is not None:
      old_raw = self.oldfc(features[:, : -self.out_dim])
      old_logits = old_raw["logits"] if isinstance(old_raw, dict) else old_raw
...
class BEEFISONet(nn.Module):
    ...
    if self.old_fc is None:
      ...
      for i in range(len(self.task_sizes) - 2, -1, -1):
        new_fc_weight = torch.cat([
            *[
                self.biases[i](
                    self.backward_prototypes.weight[i].unsqueeze(0), bias=False
                )
                for _ in range(self.task_sizes[i])
            ],
            new_fc_weight,
        ], dim=0)
        new_fc_bias = torch.cat([
            *[
                self.biases[i](
                    self.backward_prototypes.bias[i].unsqueeze(0), bias=True
                )
                for _ in range(self.task_sizes[i])
            ],
            new_fc_bias,
      ...
      fc_bias = torch.cat(
          [self.old_fc.bias, torch.zeros(new_task_size).cuda()]
      )
      fc_bias = +new_fc_bias
      ...
      for i in range(len(self.task_sizes) - 2, -1, -1):
        new_fc_weight = torch.cat(
            [self.backward_prototypes.weight[i].unsqueeze(0), new_fc_weight],
            dim=0,
        )
        new_fc_bias = torch.cat(
            [self.backward_prototypes.bias[i].unsqueeze(0), new_fc_bias]
        )
      out["train_logits"] = (
          features[:, -self.out_dim :] @ new_fc_weight.permute(1, 0)
          + new_fc_bias
      )
    out.update({
        "eval_logits": out["logits"],
        "energy_logits": self.forward_prototypes(features[:, -self.out_dim :])[
            "logits"
        ],
    })
    return out
  ...
  def update_fc_before(self, nb_classes):
    new_task_size = nb_classes - sum(self.task_sizes)
    self.biases = nn.ModuleList(
        [BiasLayer() for i in range(len(self.task_sizes))]
    )
    self.convnets.append(get_convnet(self.args))
    if self.out_dim is None:
      self.out_dim = self.convnets[-1].out_dim
    if self.new_fc is not None:
      self.fe_fc = self.generate_fc(self.out_dim, nb_classes)
      self.backward_prototypes = self.generate_fc(
          self.out_dim, len(self.task_sizes)
      )
    ...
    if self.old_fc is not None:
      ...
      for i in range(len(self.task_sizes) - 2, -1, -1):
        new_fc_weight = torch.cat([
            *[
                self.biases[i](
                    self.backward_prototypes.weight.data[i].unsqueeze(0),
                    bias=False,
                )
                for _ in range(self.task_sizes[i])
            ],
        ...
        new_fc_bias = torch.cat([
            *[
                self.biases[i](
                    self.backward_prototypes.bias.data[i].unsqueeze(0),
                    bias=True,
                )
                for _ in range(self.task_sizes[i])
            ],
      ...
      old_fc.bias.data = torch.cat(
          [self.old_fc.bias.data, torch.zeros(new_task_size).cuda()]
      )
...
class AdaptiveNet(nn.Module):
  ...
  def forward(self, x):
      ...
      # SHAP SHORT-CIRCUIT: Return main logits directly to SHAP
      ...
      out = {
          ...
          "features": features,
          "base_features": base_feature_map,
      }
  ...
  def load_checkpoint(self, args):
    ...
    pretrained_adap_dict = {
        k: v for k, v in model_dict.items() if k in adap_state_dict
    }
...
class ACILNet(BaseNet):
  ...
  def set_shap(self, mode):
      self.shap = mode
      # Unfreeze all network parameters so autograd constructs the graph
      for param in self.parameters():
...
class DSALNet(ACILNet):
  ...
  def forward(self, x):
    if getattr(self, "shap", False):
      ...
      raw_main = self.fc(self.activation_main(buf))
      out_main = (
          raw_main["logits"] if isinstance(raw_main, dict) else raw_main
      )

      raw_comp = self.fc_comp(self.activation_comp(buf))
      out_comp = (
          raw_comp["logits"] if isinstance(raw_comp, dict) else raw_comp
      )

      logits = out_main + self.C * out_comp
      return logits
    else:
      with torch.no_grad():
        feats = self.convnet(x)["features"]
        ...
        return {"logits": out_main + self.C * out_comp}
...
class TagFexNet(nn.Module):
  ...
  def __init__(self, args, pretrained):
    ...
    self.projector = nn.Sequential(
        TagFex_SimpleLinear(
            self.ta_feature_dim, self.args["proj_hidden_dim"]
        ),
        nn.ReLU(True),
        TagFex_SimpleLinear(
  ...
  def forward(self, x):
      ...
      # SHAP SHORT-CIRCUIT: Return main logits directly to GradientExplainer
      if getattr(self, "shap", False):
        return logits
      ...
      if self.trans_classifier is not None:
        ...
        trans_raw = self.trans_classifier(merged_feature)
        trans_logits = (
            trans_raw["logits"] if isinstance(trans_raw, dict) else trans_raw
        )
        out.update(trans_logits=trans_logits)

      if self.predictor is not None:
        pred_raw = self.predictor(ta_feature)
        pred_feature = (
            pred_raw["features"] if isinstance(pred_raw, dict) else pred_raw
        )
        out.update(predicted_feature=pred_feature)
  ...
  def update_fc(self, nb_classes):
    if len(self.convnets) == 0:
      ...
      for ts_old_parameter, ts_new_parameter, ta_prarameter in zip(
        ...
        ts_new_parameter.data = (
            ...
            + (1 - init_interpolation_factor) * ta_prarameter.data
        )
    ...
    self.aux_fc = self.generate_fc(self.out_dim, new_task_size + 1).to(
        self._device
    )

    if self.predictor is None:
      self.predictor = self.generate_fc(
          self.ta_feature_dim, self.ta_feature_dim
      ).to(self._device)
    if self.ts_attn is None:
      self.ts_attn = TSAttention(
          self.out_dim, self.args["attn_num_heads"], device=self._device
      )
    else:
      self.ts_attn._reset_parameters()
    self.trans_classifier = self.generate_fc(
        self.ta_net.out_dim, new_task_size
    ).to(self._device)
...
class TSAttention(nn.Module):
  ...
  def __init__(self, embed_dim, num_heads, device) -> None:
    ...
    self.weight_q = nn.Parameter(
        torch.empty((embed_dim, embed_dim), device=device)
    )
    self.weight_k_ts = nn.Parameter(
        torch.empty((embed_dim, embed_dim), device=device)
    )
    self.weight_k_ta = nn.Parameter(
        torch.empty((embed_dim, embed_dim), device=device)
    )
    self.weight_v_ts = nn.Parameter(
        torch.empty((embed_dim, embed_dim), device=device)
    )
    self.weight_v_ta = nn.Parameter(
        torch.empty((embed_dim, embed_dim), device=device)
    )
  ...
  def forward(self, ta_feats, ts_feats):
    ...
    q = (
        (ts_feats @ self.weight_q)
        .reshape(bs, N, self.num_heads, C // self.num_heads)
        .transpose(1, 2)
    )
    k_ts = (
        (ts_feats @ self.weight_k_ts)
        .reshape(bs, N, self.num_heads, C // self.num_heads)
        .transpose(1, 2)
    )
    k_ta = (
        (ta_feats @ self.weight_k_ta)
        .reshape(bs, N, self.num_heads, C // self.num_heads)
        .transpose(1, 2)
    )
    v_ts = (
        (ts_feats @ self.weight_v_ts)
        .reshape(bs, N, self.num_heads, C // self.num_heads)
        .transpose(1, 2)
    )
    v_ta = (
        (ta_feats @ self.weight_v_ta)
        .reshape(bs, N, self.num_heads, C // self.num_heads)
        .transpose(1, 2)
    )

    feat = nn.functional.scaled_dot_product_attention(
        q, torch.cat((k_ta, k_ts), dim=2), torch.cat((v_ta, v_ts), dim=2)
    )
