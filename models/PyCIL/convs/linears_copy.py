...
class SplitCosineLinear(nn.Module):
  ...
  def __init__(
      self, in_features, out_features1, out_features2, nb_proxy=1, sigma=True
  ):
...
class AnalyticLinear(torch.nn.Linear, metaclass=ABCMeta):
  ...
  def forward(self, X: torch.Tensor) -> Dict[str, torch.Tensor]:
    # REMOVED @torch.no_grad() decorator so autograd can compute SHAP gradients
    X = X.to(self.weight)
    if self.bias:
  ...
  def after_task(self) -> None:
    assert torch.isfinite(self.weight).all(), (
        'Pay attention to the numerical stability! A possible solution is to'
        ' increase the value of gamma. Setting self.dtype=torch.double also'
        ' helps.'
    )
