...
import Saliency.DGR.gan as gan
import Saliency.DGR.dgr as dgr
import Saliency.DGR.utils as utils
from Saliency.DGR.const import EPSILON
...
class WGAN(dgr.Generator):
    ...
    def train_a_batch(self, x, y, x_=None, y_=None, importance_of_new_task=.5, dataset_config=None, task=0):
        ...
        return {'c_loss': c_loss.item(), 'g_loss': g_loss.item()}, None, None, None
    ...
    def _gradient_penalty(self, x, g, lamda):
        ...
        gradients = autograd.grad(
            c, interpolated, grad_outputs=(
                torch.ones(c.size()).to(self.device)
            ),
            create_graph=True,
            retain_graph=True,
        )[0]
