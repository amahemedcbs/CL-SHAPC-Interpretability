import abc
import Saliency.DGR.utils as utils
...
class Solver(BatchTrainable):
    ...
    def train_a_batch(self, x, y, x_=None, y_=None, importance_of_new_task=.5, dataset_config=None, task=0):
        ...
        if dataset_config:
            #print(f"Values: task={task}, class_per_task={dataset_config['class_per_task']}]")
            #print(f"The range is [0:{dataset_config['class_per_task'] * (0 + task)}]")
            _, real_predicted = real_scores[:, 0: dataset_config['class_per_task'] * (1 + task)].max(1)
        else: _, real_predicted = real_scores.max(1)
        ...
        if x_ is not None and y_ is not None:
            ...
            if dataset_config:
                # print(f"Values: task={task}, class_per_task={dataset_config['class_per_task']}]")
                # print(f"The range is [0:{dataset_config['class_per_task'] * (0 + task)}]")
                _, replay_predicted = replay_scores[:, 0: dataset_config['class_per_task'] * (1 + task)].max(1)
            else:
                _, replay_predicted = real_scores.max(1)
        ...
        loss.backward()
        self.optimizer.step()
        return {'loss': loss.item(), 'precision': precision}, x, y, real_predicted
...
class Scholar(GenerativeMixin, nn.Module):
    ...
    def __init__(self, label, generator, solver, dataset_config):
        super().__init__()
        self.label = label
    ...
    def train_with_replay(
        ...
        _ = self._train_batch_trainable_with_replay(
        ...
        imgs, labels, preds = self._train_batch_trainable_with_replay(
    ...
    def _train_batch_trainable_with_replay(
            ...
            training_callbacks=None, collate_fn=None, dataset_config=None, task=0):
        ...
        for batch_index in progress:
            ...
            if x_ is not None and y_ is not None:
                x_ = Variable(x_).to(self.device)
                y_ = Variable(y_).to(self.device)

            # train the model with a batch.
            result, imgs, labels, preds = trainable.train_a_batch(
                x, y, x_=x_, y_=y_,
                importance_of_new_task=importance_of_new_task,
                dataset_config=dataset_config, task=task,
            )
    ...
    def _is_on_cuda(self):
        ...
        #return next(self.parameters()).is_xpu
