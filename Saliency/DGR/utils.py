...
def save_checkpoint(model, model_dir, task=None):
    if task is None: path = os.path.join(model_dir, model.name)
    else: path = os.path.join(model_dir, f'{model.name}_ses_{task}')
...
def validate(model, dataset, test_size=1024,
             cuda=False, verbose=True, collate_fn=None, task=None):
    ...
    for data, labels in data_loader:
        ...
        _, predicted = torch.max(scores[:, 0: 2 * (1 + task)], 1)
