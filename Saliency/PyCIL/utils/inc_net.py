...
from Saliency.PyCIL.convs.cifar_resnet import resnet32, resnet32mnist
from Saliency.PyCIL.convs.resnet import resnet18, resnet34, resnet50, resnet101, resnet152
from Saliency.PyCIL.convs.ucir_cifar_resnet import resnet32 as cosine_resnet32
from Saliency.PyCIL.convs.ucir_resnet import resnet18 as cosine_resnet18
from Saliency.PyCIL.convs.ucir_resnet import resnet34 as cosine_resnet34
from Saliency.PyCIL.convs.ucir_resnet import resnet50 as cosine_resnet50
from Saliency.PyCIL.convs.linears import SimpleLinear, SplitCosineLinear, CosineLinear,TagFex_SimpleLinear
from Saliency.PyCIL.convs.modified_represnet import resnet18_rep,resnet34_rep
from Saliency.PyCIL.convs.resnet_cbam import resnet18_cbam,resnet34_cbam,resnet50_cbam
from Saliency.PyCIL.convs.memo_resnet import  get_resnet18_imagenet as get_memo_resnet18 #for MEMO imagenet
from Saliency.PyCIL.convs.memo_cifar_resnet import get_resnet32_a2fc as get_memo_resnet32 #for MEMO cifar
from Saliency.PyCIL.convs.memo_cifar_resnet import get_resnet32mnist_a2fc as get_memo_resnet32mnist #for MEMO mnist
from Saliency.PyCIL.convs.ACL_buffer import RandomBuffer, activation_t
from Saliency.PyCIL.convs.linears import RecursiveLinear
...
class TagFexNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self._device = "cuda:0" if torch.cuda.is_available() else "cpu"#args["device"][0]
    ...
    def update_fc(self, nb_classes):
        if len(self.convnets) == 0:
            ...
            for ts_old_parameter, ts_new_parameter, ta_prarameter in zip(self.convnets[-2].parameters(),
                                                                         self.convnets[-1].parameters(),
                                                                         self.ta_net.parameters()):
                ts_new_parameter.data = init_interpolation_factor * ts_old_parameter.data + (
...
class TSAttention(nn.Module):
    ...
    def forward(self, ta_feats, ts_feats):
        ...
        q = (ts_feats @ self.weight_q).reshape(bs, N, self.num_heads, C // self.num_heads).transpose(1,
                                                                                                     2)  # (bs, H, N, Ch)
        ...
        feat = nn.functional.scaled_dot_product_attention(q, torch.cat((k_ta, k_ts), dim=2), torch.cat((v_ta, v_ts),
                                                                                                       dim=2))  # (bs, H, N, Ch) # use default scale
