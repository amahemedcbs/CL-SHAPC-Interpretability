...
class RPS_net_cifar(nn.Module):
        ...
        def shap_forward(self, x):
            return self.forward(x, self.shap_path, self.shap_last)

        # Overload the __call__ method
        def __call__(self, *args, **kwargs):
            if self.shap:
                # If in SHAP mode, call the SHAP-specific forward
                return self.shap_forward(*args, **kwargs)
            else:
                # Otherwise, call the original __call__ method
